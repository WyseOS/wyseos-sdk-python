...
- `results/latest.json`：运行时生成，只保存摘要结果，不提交。
- `results/latest.log`：运行时生成，保存调试细节，不提交。
...
- 发布文本包含 `MATE_E2E_PUBLISH_TEXT_PREFIX`、run ID 和 nonce。
- 同一轮矩阵中不复用完全相同的写入文本。
...
9. 使用返回的 `TaskResult` 做断言。
10. 对 `TaskResult` 进行分类。
11. 存储结果并继续下一个场景。

场景顺序执行。这样真实 X 账号状态和 OAuth 交互更容易理解和排查。

Runner 组装 `TaskExecutionOptions` 时必须使用前文的能力映射规则，并显式覆盖 `completion_timeout` 与 `max_user_input_timeout`。`max_user_input_timeout` 必须是有限值，避免无人值守环境中因为授权或意外输入请求永久挂死。
...
`TaskResult.final_answer`、`TaskResult.error`、`TaskResult.execution_logs` 和 exception traceback 等调试细节写入 `results/latest.log`，不平铺进 JSON 摘要。
...
调试细节写入 `examples/x_capability_e2e/results/latest.log`，包括每个场景的 `TaskResult.final_answer`、`TaskResult.error`、关键 `execution_logs` 和异常堆栈。
